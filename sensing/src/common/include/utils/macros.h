...
#define DEBUG_REL_VX       (1)

#define USE_MAIN_CAMERA_PROB_OF_EXIST      (0)

#define ADD_X_Y_LIMITATION_FOR_AROUND_RADAR (0)


/*
* 是否使用新的AD_HMI录制的数据也就是5月15日之后用新版AD_HMI录制的数据包
*/
#define  USING_NEW_ADHMI_ROSBAG_DATA         (1)

/**
 * @brief 调试adecu的融合结果时，设置的ID值，用来对
 * 
 */
#define ADECU_FUSION_DEBUG_TARGET_ID_0 (21)
#define ADECU_FUSION_DEBUG_TARGET_ID_1 (2)
#define ADECU_FUSION_DEBUG_TARGET_ID_2 (0)
#define ADECU_FUSION_DEBUG_TARGET_ID_3 (0)
...
/// 定义是否进入回放模式(AD_HMI数据)
#define ENTER_PLAYBACK_MODE_AD_HMI (0)

/// 定义是否发送ADHMI融合数据(如果发送ADHMI融合数据，将不会接受sensing_fusion的融合数据)
#define ENTER_PLAYBACK_MODE_AD_HMI_FUSION (0)

/// 定义是否把数据保存到csv文件
#define ENTER_OBJ_INFO_TO_CSV (0)
