...
void WorkMonitor::Initialize() {
  ...
  monitor_table_.recv_msg_visual_control_lane_mark_camera.Initialize(
        INTERNAL_MODULE_ID_MSG_RECV_VISUAL_CONTROL_LANE, 4);
  ...
  monitor_table_.recv_msg_adhmi_obstacle_fusion.Initialize(
        INTERNAL_MODULE_ID_MSG_RECV_ADHMI_FUSION_OBJ, 4);
}
...
void WorkMonitor::FeedDog_RecvVisualControlLaneMarkCamera() {
  monitor_table_.recv_msg_visual_control_lane_mark_camera.FeedDog(
        0, ad_msg::MODULE_STATUS_OK, 0, 0, 0);
}
...
void WorkMonitor::FeedDog_RecvADHMIObstacleFusionObj() {
  monitor_table_.recv_msg_adhmi_obstacle_fusion.FeedDog(
        0, ad_msg::MODULE_STATUS_OK, 0, 0, 0);
}
...
Int32_t WorkMonitor::DoWork() {
  ...
  // message receive (visual control lane mark camera)
  monitor_table_.recv_msg_visual_control_lane_mark_camera.UpdateStatus(&module_status);
  internal_module_status_list_.PushBackModuleStaus(module_status);
  ...
  // message receive (mpu state)
  monitor_table_.recv_msg_mpu_state.UpdateStatus(&module_status);
  internal_module_status_list_.PushBackModuleStaus(module_status);


  // message receive (adhmi fusion obstacle)
  monitor_table_.recv_msg_adhmi_obstacle_fusion.UpdateStatus(&module_status);
  internal_module_status_list_.PushBackModuleStaus(module_status);
