...
// for pausing process
static boost::condition_variable s_process_pause_cond;
// handle sig_int
...
//  }
  ...
  // Start task manager
  bool ret = task_manager->Start();
  if (false == ret) {
    LOG_ERR << "Failed to start task manager.";

#if (PROJECT_PLATFORM == PROJECT_PLATFORM_LINUX) || \
    (PROJECT_PLATFORM == PROJECT_PLATFORM_WINDOWS)
    QMessageBox::critical(Nullptr_t, "Vehicle Motion Planning",
        "Failed to start task manager, close windows, "
        "check error log, and retry!",
