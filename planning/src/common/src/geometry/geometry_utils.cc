...
geo_var_t CalcCurvatureByTriEdge(
  ...
  if ((com_abs(b + c - a) < kGeometryEpsilon) ||
    ...
    // 边长不满足三角形的性质
    // 两条边的和不小于第三条边
    LOG_ERR << "Invalid triangle edges.";
  } else if ((a * b * c) < 2*kGeometryEpsilon) {
    curvature = 0.0F;
    LOG_ERR << "Invalid triangle edges.";
  } else {
    curvature = (4.0F * com_sqrt(p * (p - a) * (p - b) * (p - c))) / (a * b * c);
  }
...
geo_var_t CalcCurvatureByTriCorner(
  ...
  return (-Orient_2D(a, c, b) * CalcCurvatureByTriEdge(side_a, side_b, side_c));
}
