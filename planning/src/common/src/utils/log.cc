...
LogStream& LogStream::operator << (const std::string& data) {
  Int32_t str_len = data.size();
  Int32_t remanent_space = static_cast<Int32_t>(MAX_BUFF_SIZE) - count_ - 1;

  if (remanent_space > str_len) {
    com_snprintf(&str_buff_[count_], static_cast<Uint32_t>(remanent_space), "%s", data.c_str());
    count_ += str_len;
    str_buff_[count_] = '\0';
  }
...
Int32_t LogFile::s_file_obj_count_ = 0;
...
void LogFile::Initialize(const Char_t* module_name) {
  s_file_obj_count_++;
  ...
  if (Nullptr_t != module_name) {
    ...
    com_snprintf(module_name_, static_cast<Uint32_t>(MAX_MODULE_NAME_SIZE) - 1U,
                 "null_name_%d", s_file_obj_count_);
  }
...
void LogFile::SetModuleName(const Char_t* module_name) {
  if (Nullptr_t != module_name) {
    com_snprintf(module_name_, static_cast<Uint32_t>(MAX_MODULE_NAME_SIZE) - 1U,
                 "%s", module_name);
  } else {
