...
public:
  ...
  /**
   * @brief 将std::string型数据输入到缓冲区
   * @param[in] std::string型数据
   * @return 日志流的引用
   */
...
public:
   ...
   * @param[in] module_name 模块名称（在文件名称中体现）
   */
  void SetModuleName(const Char_t* module_name);

  /**
   * @brief 文件是否已打开
   * @return true-已打开，false-未打开
   */
  bool IsOpen(void) { return (file_is_open_); }
...
private:
   ...
   * @param[in] module_name 模块名称（在文件名称中体现）
   */
  void Initialize(const Char_t* module_name);
...
private:
  ...
  // 文件对象的数量
  static Int32_t s_file_obj_count_;
  // 是否允许输出到文件
  bool enable_;
  // 文件是否已经打开
  bool file_is_open_;
