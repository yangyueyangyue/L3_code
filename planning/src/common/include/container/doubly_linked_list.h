...
public:
  ...
  /**
   * @brief 在链表中插入数据
   * @param[in] rv 插入位置（在其前面）
   * @param[in] new_data 插入的数据
   * @param[in] pool 链表元素存储区域的引用
   * @return 指向新数据的迭代器
   */
  iterator Insert(const iterator& rv, const data_type& new_data, pool_type& pool);
   ...
   * @return 更新后的迭代器（指向被移除的数据的下一个数据）
   */
  iterator Erase(const iterator& rv, pool_type& pool);
...
template <typename DataType, Int32_t MaxDataNum>
typename DoublyLinkedList<DataType, MaxDataNum>::iterator
DoublyLinkedList<DataType, MaxDataNum>::Insert(
    const iterator& rv, const data_type& new_data, pool_type& pool) {
  iterator it_ret = rv;
  if (!pool.IsNull(rv.index_)) {
    node_type* addr = pool.GetVirtualAddr(rv.index_);
    node_type* node = AllocateListNode(pool);
    if (Nullptr_t != node) {
      Insert(pool.GetVirtualAddr(addr->prev), node, pool);
      node->data = new_data;
      it_ret.index_ = addr->prev;
    }
  } else {
    node_type* node = AllocateListNode(pool);
    if (Nullptr_t != node) {
...
template <typename DataType, Int32_t MaxDataNum>
typename DoublyLinkedList<DataType, MaxDataNum>::iterator
DoublyLinkedList<DataType, MaxDataNum>::Erase(
    const iterator& rv, pool_type& pool) {
  iterator it_ret = rv;
  if (!pool.IsNull(rv.index_)) {
    node_type* addr = pool.GetVirtualAddr(rv.index_);
    ++it_ret;
    EraseListNode(addr, pool);
  }
  return (it_ret);
}
