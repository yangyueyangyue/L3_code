...
/**
 * @brief 从三角形的三条边长计算其外接圆的曲率(absolute), \n
 *        zero if abc is degenerate.
 * @param a [in] edge of triangle abc
 * @param b [in] edge of triangle abc
 * @param c [in] edge of triangle abc
 * @return 三角形的外接圆的曲率
 *
 */
geo_var_t CalcCurvatureByTriEdge(
    geo_var_t a, geo_var_t b, geo_var_t c);

/**
 * @brief 从三角形的三个顶点计算其外接圆的曲率, The result is positive \n
 *        if abc is cw, negative if abc is ccw, zero if abc is degenerate.
 * @param a [in] corner of triangle abc
 * @param b [in] corner of triangle abc
 * @param c [in] corner of triangle abc
 * @return 三角形的外接圆的曲率
 *
 */
geo_var_t CalcCurvatureByTriCorner(
    const Vec2d& a, const Vec2d& b, const Vec2d& c);
