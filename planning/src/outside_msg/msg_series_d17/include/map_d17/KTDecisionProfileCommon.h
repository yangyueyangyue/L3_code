...
namespace decision_map_t
{
    ...
    typedef struct tagsLinearItem
    {
        ...
        LineMarking m_marking =
            LineMarking::NA; ///< @brief     线型物体的标记类型
                             ///< @details   仅类型为边线时有效
                             ///<            仅输出首个值，建议使用 @ref ProfileType:LineMarkingType 获取完整列表
        LineColor m_color =
            LineColor::Unknown; ///< @brief     线型物体的颜色
                                ///< @details   仅类型为边线时有效
    ...
    typedef struct tagsAbsolutePositionMessage
    {
        ...
        uint32_t m_confidence; ///< 车辆位置的置信度
    ...
    typedef struct tagsDecisionPosition
    {
        ...
        void Clear()
        {
            s_AbsolutePosition.m_timestamp = 0;
            s_AbsolutePosition.m_datumTimestamp = 0;
            s_AbsolutePosition.m_lastTimestamp = 0;
            s_AbsolutePosition.m_heading = 0.0f;
            s_AbsolutePosition.m_speed = 0.0f;
            s_AbsolutePosition.m_oddStatus = ODDStatusType::Unknown;

            s_PostionMessage.m_timestamp = 0;
            s_PostionMessage.m_positionAge = UINT64_MAX;
            s_PostionMessage.m_datumTimestamp = 0;
            s_PostionMessage.m_oddStatus = ODDStatusType::Unknown;
            std::vector<MatchedPosition>().swap(s_PostionMessage.m_positions);
            s_PostionMessage.m_positions.clear();
        }
    } sDecisionPosition;
    ...
    typedef struct tagsDecisionMapLinearAttribute
    {
        ...
        std::map<LineId, LineControlPoints<PhysicalCrossType, PhysicalCrossType::Unknown>>
            s_mLaneRightLinePhysicalCross;
        ...
        /// huic1116
        template <typename T, T default_> void clear_item_map(std::map<LineId, LineControlPoints<T, default_>> &item__)
        {
        ...
        template <typename T, T default_> void clear_item(LineControlPoints<T, default_> &line_control_points)
        {
    ...
    typedef struct tagsDecisionMapLaneAttribute
    {
        ...
        void clear() // delete by huic
        {
            ...
            std::vector<stLaneSpeedValue>().swap(s_vLaneLimitSpeedMax);
            s_vLaneLimitSpeedMax.clear();
            std::vector<stLaneSpeedValue>().swap(s_vLaneLimitSpeedMin);
            s_vLaneLimitSpeedMin.clear();
        }
        /// huic1116
    ...
    typedef struct tagsDecisionMapLaneItem
    {
        ...
        //道路类型
        std::vector<FormOfWay> s_vFormOfWay;
        std::vector<RoadAttribute> s_vRoadAttribute;
        /// huic1116
        void clear() // delete by huic
        {
            ...
            std::vector<FormOfWay>().swap(s_vFormOfWay);
            s_vFormOfWay.clear();

            std::vector<RoadAttribute>().swap(s_vRoadAttribute);
            s_vRoadAttribute.clear();
