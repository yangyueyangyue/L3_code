...
/// 使能道路边界碰撞测试
#define ENABLE_ROAD_BOUNDARY_COLLISION_TEST (1)
 ...
 * @brief 驾驶地图的最大数量
 */
enum { MAX_DRIVING_MAP_IMPL_NUM = 2 };

/**
 * @enum MAX_LANE_NUM
 * @brief 内部地图中可以保存的最多车道的数量
 */
enum { MAX_LANE_NUM = 40 };
 ...
 */
enum {
  GNSS_COORDINATE_TYPE_UTM,
  GNSS_COORDINATE_TYPE_ODOM
};
...
struct DrivingMapConfig {
   ...
   */
  DrivingMapConfig() {
    inputted_map_type = INPUTTED_MAP_TYPE_MIXED;
  }
};
...
struct DrivingMapDataSource {
  ...
  DrivingMapDataSource() {
    timestamp = 0;
    gnss_coordinate_type = GNSS_COORDINATE_TYPE_UTM,
    gnss = Nullptr_t;
...
struct DrivingMapInfo {
  ...
  /// 道路边界Obj列表
  common::StaticVector<common::OBBox2d,
  MAX_OBJECT_NUM_IN_OBJ_SPACE> road_boundary_obj_list;
  ...
  ad_msg::SceneStoryList::MAX_SCENE_STORYS_NUM> scene_storys;
   ...
   */
  void Clear() {
    driving_map_type = DRIVING_MAP_TYPE_INVALID;
    ...
    for (Int32_t i = 0; i < road_boundary_obj_list.Size(); ++i) {
      road_boundary_obj_list[i].Clear();
    }
    road_boundary_obj_list.Clear();
