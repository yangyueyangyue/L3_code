...
Int32_t EncodePlanningResultArray(
  ...
  for (Int32_t i = 0; i < elements; ++i) {
    ...
    thislen = common::EncodeValueArray(buf, offset + pos, maxlen - pos,
                                       &(p[i].planning_status[0]), 4);
    if (thislen < 0) {
      LOG_ERR << "Failed to encode \"planning_status\" of PlanningResult[" << i << "].";
      return (thislen);
    } else {
      pos += thislen;
    }
    ...
    if (thislen < 0) {
      LOG_ERR << "Failed to encode \"steering_wheel_speed\" of PlanningResult[" << i << "].";
      return (thislen);
    } else {
      pos += thislen;
    }
...
Int32_t DecodePlanningResultArray(
  ...
  for (Int32_t i = 0; i < elements; i++) {
    ...
    thislen = common::DecodeValueArray(buf, offset + pos, maxlen - pos,
                                       &(p[i].planning_status[0]), 4);
    if (thislen < 0) {
      LOG_ERR << "Failed to decode \"planning_status\" of PlanningResult[" << i << "].";
      return (thislen);
    } else {
      pos += thislen;
    }
    ...
    thislen = common::DecodeValueArray(buf, offset + pos, maxlen - pos,
                                       &(p[i].steering_wheel_speed), 1);
    if (thislen < 0) {
      LOG_ERR << "Failed to decode \"steering_wheel_speed\" of PlanningResult[" << i << "].";
      return (thislen);
    } else {
      pos += thislen;
    }
