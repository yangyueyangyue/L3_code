...
bool ParseProtoMsg::EncodeChassisMessage(
  ...
  case (ad_msg::VEH_TRAILER_STATUS_NOT_CONNECTED):
    data_out->set_trailer_status(msg::control::Chassis::TRAILER_STATUS_NOT_CONNECTED);
    break;
  case (ad_msg::VEH_TRAILER_STATUS_CONNECTED):
...
bool ParseProtoMsg::DecodeChassisMessage(
  ...
  case (msg::control::Chassis::TRAILER_STATUS_NOT_CONNECTED ):
    data_out->trailer_status = ad_msg::VEH_TRAILER_STATUS_NOT_CONNECTED;
    break;
  case (msg::control::Chassis::TRAILER_STATUS_CONNECTED ):
    data_out->trailer_status = ad_msg::VEH_TRAILER_STATUS_CONNECTED;
    break;
  default:
    data_out->trailer_status = ad_msg::VEH_TRAILER_STATUS_NOT_CONNECTED;
    break;
...
bool ParseProtoMsg::EncodeObstacleListMessage(
  ...
  for (Int32_t i = 0; i < msg.obstacle_num; ++i) {
    msg::perception::Obstacle* dst =
        data_out->add_obstacles();
    if (Nullptr_t != dst) {
      const ad_msg::Obstacle& obj_in = msg.obstacles[i];
      dst->set_id(obj_in.id);
      dst->set_x(obj_in.x);
      dst->set_y(obj_in.y);
      msg::perception::Obstacle::OBBox* obb = dst->mutable_obb();
      if (Nullptr_t != obb) {
        obb->set_x(obj_in.obb.x);
        obb->set_y(obj_in.obb.y);
        obb->set_heading(obj_in.obb.heading);
        obb->set_half_length(obj_in.obb.half_length);
        obb->set_half_width(obj_in.obb.half_width);
      } else {
        LOG_ERR << "Failed to add obb.";
      }
      dst->set_height(obj_in.height);
      dst->set_height_to_ground(obj_in.height_to_ground);
      switch (obj_in.type) {
      case (ad_msg::OBJ_TYPE_UNKNOWN):
      ...
      dst->set_dynamic(obj_in.dynamic);
      dst->set_confidence(obj_in.confidence);
      switch (obj_in.perception_type) {
      case (ad_msg::OBJ_PRCP_TYPE_UNKNOWN):
      ...
      dst->set_v_x(obj_in.v_x);
      dst->set_v_y(obj_in.v_y);
      dst->set_v(obj_in.v);
      dst->set_a_y(obj_in.a_y);
      dst->set_a_x(obj_in.a_x);
      dst->set_a(obj_in.a);

      for (Int8_t j = 0; j < obj_in.pred_path_num; ++j) {
        msg::perception::Obstacle::PredPath* pred_path =
            dst->add_pred_path();
        if (Nullptr_t != pred_path) {
          for (Int32_t k = 0; k < obj_in.pred_path_point_num[j]; ++k) {
            msg::perception::Obstacle::PredPathPoint* pred_point =
                pred_path->add_points();
            if (Nullptr_t != pred_point) {
              pred_point->set_x(obj_in.pred_path[j][k].x);
              pred_point->set_y(obj_in.pred_path[j][k].y);
              pred_point->set_heading(obj_in.pred_path[j][k].heading);
              pred_point->set_s(obj_in.pred_path[j][k].s);
            } else {
              LOG_ERR << "Failed to add pred_path point.";
            }
          }
        } else {
          LOG_ERR << "Failed to add pred path.";
        }
      }

      for (Int8_t i = 0; i < obj_in.tracked_path_point_num; ++i) {
        msg::perception::Obstacle::Point2D* point_2d =
            dst->add_tracked_path();
        if (Nullptr_t != point_2d) {
          point_2d->set_x(obj_in.tracked_path[i].x);
          point_2d->set_y(obj_in.tracked_path[i].y);
        } else {
          LOG_ERR << "Failed to add tracked path.";
        }
      }
    } else {
      LOG_ERR << "Failed to add Obstacle.";
    }
  }
...
#else
  LOG_ERR << "Invalid TrafficSignalSpeedRestriction ID Type.";
#endif
  // 限速标志类型
  switch (msg.type) {
...
bool ParseProtoMsg::EncodePlanningResultMessage(
  ...
  data_out->set_tar_throttle(msg.tar_throttle);
  
  data_out->set_ramp_slope_value(msg.ramp_slope_value);
  ...
  // 编码：拟合轨迹点的系数
    ...
    msg::planning::PlanningResult::TarTrj::PlanningTraj_Cv* traj_coeffs =
          tar_trj->mutable_traj_coeffs();

    traj_coeffs->set_traj_cv0(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv0);
    traj_coeffs->set_traj_cv1(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv1);
    traj_coeffs->set_traj_cv2(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv2);
    traj_coeffs->set_traj_cv3(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv3);
    traj_coeffs->set_traj_cv4(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv4);
    traj_coeffs->set_traj_cv5(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Cv5);
    traj_coeffs->set_traj_dist2pathzero(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_Dist2PathZero);
    traj_coeffs->set_traj_isvalid(msg.tar_trj.PlanningTraj_Cv.PlanningTraj_IsValid);

    for (Int32_t i = 0; i < 6; ++i) {
      traj_coeffs->add_traj_coeffs(msg.tar_trj.PlanningTraj_Cv.coeffs[i]);//add_
    }

    // 编码：车道中心线的系数
    msg::planning::PlanningResult::TarTrj::LaneTraj_Cv* lane_coeffs =
        tar_trj->mutable_lane_coeffs();

    lane_coeffs->set_lane_cv0(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv0);
    lane_coeffs->set_lane_cv1(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv1);
    lane_coeffs->set_lane_cv2(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv2);
    lane_coeffs->set_lane_cv3(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv3);
    lane_coeffs->set_lane_cv4(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv4);
    lane_coeffs->set_lane_cv5(msg.tar_trj.LaneTraj_Cv.LaneTraj_Cv5);
    lane_coeffs->set_lane_dist2path_zero(msg.tar_trj.LaneTraj_Cv.LaneDist2PathZero);
    lane_coeffs->set_lane_isvalid(msg.tar_trj.LaneTraj_Cv.LaneTraj_IsValid);

    for (Int32_t i = 0; i < 6; ++i) {
      lane_coeffs->add_lane_coeffs(msg.tar_trj.LaneTraj_Cv.c_center[i]);//add_
    }

   // 编码：车道左边线的系数
    msg::planning::PlanningResult::TarTrj::LeftLaneTraj_Cv* left_lane_coeffs =
      tar_trj->mutable_left_lane_coeffs();

    left_lane_coeffs->set_left_lane_cv0(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv0);
    left_lane_coeffs->set_left_lane_cv1(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv1);
    left_lane_coeffs->set_left_lane_cv2(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv2);
    left_lane_coeffs->set_left_lane_cv3(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv3);
    left_lane_coeffs->set_left_lane_cv4(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv4);
    left_lane_coeffs->set_left_lane_cv5(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_Cv5);
    left_lane_coeffs->set_left_lane_dist2path_zero(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneDist2PathZero);
    left_lane_coeffs->set_left_lane_isvalid(msg.tar_trj.LeftLaneTraj_Cv.LeftLaneTraj_IsValid);

    for (Int32_t i = 0; i < 6; ++i) {
      left_lane_coeffs->add_left_lane_coeffs(msg.tar_trj.LeftLaneTraj_Cv.c_left[i]);//add_
    }


    // 编码：车道右边线的系数
    msg::planning::PlanningResult::TarTrj::RightLaneTraj_Cv* right_lane_coeffs =
        tar_trj->mutable_right_lane_coeffs();

  right_lane_coeffs->set_right_lane_cv0(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv0);
  right_lane_coeffs->set_right_lane_cv1(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv1);
  right_lane_coeffs->set_right_lane_cv2(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv2);
  right_lane_coeffs->set_right_lane_cv3(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv3);
  right_lane_coeffs->set_right_lane_cv4(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv4);
  right_lane_coeffs->set_right_lane_cv5(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_Cv5);
  right_lane_coeffs->set_right_lane_dist2path_zero(msg.tar_trj.RightLaneTraj_Cv.RightLaneDist2PathZero);
  right_lane_coeffs->set_right_lane_isvalid(msg.tar_trj.RightLaneTraj_Cv.RightLaneTraj_IsValid);

    for (Int32_t i = 0; i < 6; ++i) {
      right_lane_coeffs->add_right_lane_coeffs(msg.tar_trj.RightLaneTraj_Cv.c_right[i]);//add_
    }
#endif
...
bool ParseProtoMsg::DecodePlanningResultMessage(
  ...
  // 拟合轨迹点的系数
  // const msg::planning::PlanningResult::TarTrj::PlanningTraj_Cv&  trj_coeffs=
  //     tar_trj.trj_coeffs();
  // data_out->tar_trj.trj_coeffs.coeffs[0] = PlanningTraj_Cv.coeffs();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv0= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv0();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv1= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv1();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv2= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv2();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv3= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv3();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv4= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv4();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Cv5= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Cv5();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_Dist2PathZero= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_Dist2PathZero();
  // data_out->tar_trj.trj_coeffs.PlanningTraj_IsValid= PlanningTraj_Cv.PlanningTraj_Cv.PlanningTraj_IsValid();
...
bool ParseProtoMsg::DecodeSceneStoryControlLine(
    const msg::routing::SceneStoryControlLine& line_in,
    ad_msg::SceneStoryControlLine* line_out) {
  line_out->start_point.x = line_in.start_point().x();
  line_out->start_point.y = line_in.start_point().y();
  line_out->end_point.x = line_in.end_point().x();
  line_out->end_point.y = line_in.end_point().y();

  return true;
}

bool ParseProtoMsg::DecodeSceneStoryArea(
    const msg::routing::SceneStoryArea& area_in,
    ad_msg::SceneStoryArea* area_out) {
  if (area_in.has_area_type_01()) {
    area_out->area_type = ad_msg::SceneStoryArea::AREA_TYPE_01;
    if (area_in.area_type_01().has_distance()) {
      area_out->area_type_01.valid = true;
      area_out->area_type_01.distance = area_in.area_type_01().distance();
    } else {
      area_out->area_type_01.Clear();
    }
  } else if (area_in.has_area_type_02()) {
    area_out->area_type = ad_msg::SceneStoryArea::AREA_TYPE_02;

    area_out->area_type_02.valid = false;
    if (area_in.area_type_02().has_control_line()) {
      if (area_in.area_type_02().control_line().has_start_point() &&
          area_in.area_type_02().control_line().has_end_point()) {
        area_out->area_type_02.valid = true;

        DecodeSceneStoryControlLine(area_in.area_type_02().control_line(),
                                    &(area_out->area_type_02.control_line));
        if (area_in.area_type_02().has_start_s()) {
          area_out->area_type_02.start_s = area_in.area_type_02().start_s();
        } else {
          area_out->area_type_02.start_s = 0.01F;
        }
        if (area_in.area_type_02().has_end_s()) {
          area_out->area_type_02.end_s = area_in.area_type_02().end_s();
        } else {
          area_out->area_type_02.end_s = -0.01F;
        }
      }
    }
    if (!area_out->area_type_02.valid) {
      area_out->area_type_02.Clear();
    }
  } else {
    area_out->area_type = ad_msg::SceneStoryArea::AREA_TYPE_INVALID;
...
bool ParseProtoMsg::DecodeSceneStoryCondition(
    const msg::routing::SceneStoryCondition& cond_in,
    ad_msg::SceneStoryCondition* cond_out) {
  if (cond_in.has_valid_area()) {
    cond_out->valid_area = cond_in.valid_area();
  } else {
    cond_out->valid_area = false;
  }
  ...
  if (cond_in.has_gear()) {
    switch (cond_in.gear()) {
    case (msg::control::Chassis::GEAR_P) :
      cond_out->gear = ad_msg::VEH_GEAR_P;
      break;
    case (msg::control::Chassis::GEAR_N) :
      cond_out->gear = ad_msg::VEH_GEAR_N;
      break;
    case (msg::control::Chassis::GEAR_R) :
      cond_out->gear = ad_msg::VEH_GEAR_R;
      break;
    case (msg::control::Chassis::GEAR_D) :
      cond_out->gear = ad_msg::VEH_GEAR_D;
      break;
    default:
...
bool ParseProtoMsg::DecodeSceneStoryAction(
    const msg::routing::SceneStoryAction& action_in,
    ad_msg::SceneStoryAction* action_out) {
  if (action_in.has_hold_time()) {
    action_out->hold_time = action_in.hold_time() * 1000;
  } else {
    action_out->hold_time = -1;
  }
...
bool ParseProtoMsg::DecodeSceneStorysMessage(
  ...
  // parse
  msg::routing::SceneStories scene_storys;
  ...
  for (Int32_t i = 0; i < story_size; ++i) {
    ...
    if (data_in.has_area()) {
      DecodeSceneStoryArea(data_in.area(), &(story.area));
    } else {
      story.area.Clear();
    }
...
#else
  LOG_ERR << "Invalid MapLocalization ID Type.";
#endif
...
void ParseProtoMsg::DecodePlanningStoryRefLine(
    const msg::planning::PlanningStoryRefLine& ref_in,
    ad_msg::PlanningStoryRefLine* ref_out) {
  Int32_t dst_point_idx = ref_out->point_num;
  Int32_t point_num = ref_in.points_size();
  for (Int32_t i = 0; i < point_num; ++i) {
    if (dst_point_idx >= ad_msg::PlanningStoryRefLine::MAX_POINT_NUM) {
      break;
    }
    const msg::planning::PlanningStoryRefLine::Point& src_point =
        ref_in.points(i);
    ad_msg::PlanningStoryRefLine::Point& dst_point =
        ref_out->points[dst_point_idx];
    dst_point_idx++;

    dst_point.x = src_point.x();
    dst_point.y = src_point.y();
  }
  ref_out->point_num = dst_point_idx;
}

void ParseProtoMsg::DecodePlanngingStoryCondition(
    const msg::planning::PlanningStoryCondition& cond_in,
    ad_msg::PlanningStoryCondition* cond_out) {
  if (cond_in.has_area()) {
    cond_out->area.valid = true;
    const msg::planning::PlanningStoryCondition::Area& area_in = cond_in.area();
    if (area_in.has_left_width()) {
      cond_out->area.left_width = area_in.left_width();
    } else {
      cond_out->area.left_width = -0.1F;
    }
    if (area_in.has_right_width()) {
      cond_out->area.right_width = area_in.right_width();
    } else {
      cond_out->area.right_width = -0.1F;
    }
    if (area_in.has_start_s()) {
      cond_out->area.start_s = area_in.start_s();
    } else {
      cond_out->area.start_s = -0.1F;
    }
    if (area_in.has_end_s()) {
      cond_out->area.end_s = area_in.end_s();
    } else {
      cond_out->area.end_s = -0.1F;
    }
  } else {
    cond_out->area.valid = false;
    cond_out->area.left_width = -0.1F;
    cond_out->area.right_width = -0.1F;
    cond_out->area.start_s = -0.1F;
    cond_out->area.end_s = -0.1F;
  ...
  if (cond_in.has_speed_high()) {
    cond_out->valid_speed_high = 1;
    cond_out->speed_high = cond_in.speed_high() / 3.6F;
  } else {
    cond_out->valid_speed_high = 0;
    cond_out->speed_high = 1000.0F/3.6F;
  }
  if (cond_in.has_speed_low()) {
    cond_out->valid_speed_low = 1;
    cond_out->speed_low = cond_in.speed_low() / 3.6F;
  } else {
    cond_out->valid_speed_low = 0;
    cond_out->speed_low = -1000.0F/3.6F;
  }
...
void ParseProtoMsg::DecodePlanningStoryAction(
    const msg::planning::PlanningStoryAction& action_in,
    ad_msg::PlanningStoryAction* action_out) {
  if (action_in.has_holding_time()) {
    action_out->holding_time = action_in.holding_time() * 1000;
  } else {
  ...
  if (action_in.has_speed()) {
    action_out->valid_speed = 1;
    action_out->speed = action_in.speed() / 3.6F;
  } else {
    action_out->valid_speed = 0;
    action_out->speed = 0.0F;
  }
  ...
  if (action_in.has_cmd()) {
    action_out->cmd = action_in.cmd();
  } else {
    action_out->cmd = 0;
  }
}

void ParseProtoMsg::DecodePlanningStory(
    const msg::planning::PlanningStory& story_in,
    ad_msg::PlanningStory* story_out) {
  if (story_in.has_id()) {
    story_out->id = story_in.id();
  } else {
    story_out->id = -1;
  }
  if (story_in.has_type()) {
    story_out->type = story_in.type();
  } else {
    story_out->type = 0;
  }

  DecodePlanningStoryRefLine(story_in.ref_line(), &story_out->ref_line);

  if (story_in.has_condition()) {
    story_out->condition.vaild = true;
    DecodePlanngingStoryCondition(story_in.condition(), &story_out->condition);
  } else {
    story_out->condition.vaild = false;
  }

  if (story_in.has_action()) {
    story_out->action.vaild = true;
    DecodePlanningStoryAction(story_in.action(), &story_out->action);
  } else {
    story_out->action.vaild = false;
  }
}

bool ParseProtoMsg::DecodePlanningStoryList(
    const msg::planning::PlanningStoryList& msg,
    ad_msg::PlanningStoryList* data_out) {
  data_out->Clear();

  Int32_t dst_story_idx = data_out->story_num;
  Int32_t storys_size = msg.storys_size();
  for (Int32_t i = 0; i < storys_size; ++i) {
    const msg::planning::PlanningStory& src_story = msg.storys(i);
    if (!src_story.has_ref_line()) {
      continue;
    }
    if (src_story.ref_line().points_size() < 2) {
      continue;
    }

    if (dst_story_idx >= ad_msg::PlanningStoryList::MAX_PLANNING_STORYS_NUM) {
      break;
    }
    DecodePlanningStory(src_story, &data_out->storys[dst_story_idx]);
    dst_story_idx++;
  }
  data_out->story_num = dst_story_idx;

  return true;
}
// Adecu planning debug
bool ParseProtoMsg::DecodeAdecuPlanningMessage(
    const Char_t *msg, Int32_t msg_len,
    planning::VelocityPlanningResult *data_out) {
  // parse
  ad_data_upload::Spd_Pln_Display_Data_message adecu_velocity_result;
  if (!adecu_velocity_result.ParseFromArray(msg, msg_len)) {
    LOG_ERR << "Failed to parse adcue msg from array.";
    return false;
  }
  data_out->tar_v = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_v();
  data_out->tar_a = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_a();
  data_out->tar_type = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_type();
  data_out->tar_obj.dist_to_obj = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().dist_to_obj();

  data_out->tar_obj.obj_dec_status = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().obj_dec_status();
  data_out->tar_obj.valid = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().valid();
  data_out->tar_obj.time_gap = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().time_gap();
  data_out->tar_obj.relative_v = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().relative_v();
  data_out->tar_obj.ttc = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().ttc();
  data_out->tar_obj.obj_v = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().obj_v();
  data_out->tar_obj.dist_gap_level = adecu_velocity_result.speed_planning_result_info().velocity_planning_result().tar_obj().dist_gap_level();
