...
void WidgetMap::DrawCoordinateAxis(
  ...
  // std::cout << "renderText x:" << x+len << " y:" << y << " z:" << z << std::endl;
...
// 画叉
void WidgetMap::DrawCross10(Float64_t x, Float64_t y, Float64_t len) {
  phoenix::common::Vec2d begin1;
  phoenix::common::Vec2d begin2;
  phoenix::common::Vec2d end1;
  phoenix::common::Vec2d end2;
...
void WidgetMap::UpdateModuleInfo() {
  ...
  shared_data->GetObstacleMaxieyeCameraListAfterPreprocess(&module_info_.obstacle_maxieye_camera_after_preprocess_list);
  ...
  shared_data->GetObstacleRadarLeftFwFilterList(&module_info_.obstacle_radar_left_fw_filter_list);
  shared_data->GetObstacleRadarRightFwFilterList(&module_info_.obstacle_radar_right_fw_filter_list);
  shared_data->GetObstacleRadarLeftBwFilterList(&module_info_.obstacle_radar_left_bw_filter_list);
  shared_data->GetObstacleRadarRightBwFilterList(&module_info_.obstacle_radar_right_bw_filter_list);
  ...
  shared_data->GetObstacleJMFrontList(&module_info_.obstacle_jm_front_list);
  shared_data->GetObstacleJMFrontLeftList(&module_info_.obstacle_jm_front_left_list);
  shared_data->GetObstacleJMFrontRightList(&module_info_.obstacle_jm_front_right_list);
  shared_data->GetObstacleJMRearLeftList(&module_info_.obstacle_jm_rear_left_list);
  shared_data->GetObstacleJMRearRightList(&module_info_.obstacle_jm_rear_right_list);
  shared_data->GetObstacleJMRearList(&module_info_.obstacle_jm_rear_list);
...
void WidgetMap::DrawMap() {
  ...
  DrawLaneCurbCameraList();
  DrawObstacleCameraList();
  DrawObstacleEsrFrontList();
  DrawObstacleMaxieyeCameraList();
  DrawObstacleMaxieyeCameraAfterPreprocessList();
  DrawObstacleLidarListFront();

#if ENABLE_DISPLAY_AROUND_RADAR
  DrawObstacleAnngicRadarFrontLeftList();
  DrawObstacleAnngicRadarFrontRightList();
  DrawObstacleAnngicRadarRearLeftList();
  DrawObstacleAnngicRadarRearRightList();
#endif

#if ENABLE_DISPLAY_PREPROCESS_AROUND_RADAR
  DrawObstacleRadarLeftFwList();
  DrawObstacleRadarRightFwList();
  DrawObstacleRadarLeftBwList();
  DrawObstacleRadarRightBwList();
#endif

#if ENABLE_DISPLAY_VISUAL_CONTROL
  DrawObstacleVisualControlFrontList();
  DrawObstacleVisualControlFrontLeftList();
  DrawObstacleVisualControlFrontRightList();
  DrawObstacleVisualControlRearLeftList();
  DrawObstacleVisualControlRearRightList();
  DrawObstacleVisualControlRearList();
#endif

#if ENABLE_DISPLAY_PREPROCESS_JM
  DrawObstacleJMFrontList();
  DrawObstacleJMFrontLeftList();
  DrawObstacleJMFrontRightList();
  DrawObstacleJMRearLeftList();
  DrawObstacleJMRearRightList();
#endif


  DrawObstacleFusionList();
}
...
void WidgetMap::DrawLaneMarkCameraList() {
  ...
  for (Int32_t i = 0; i < lane_mark_num; ++i) {
    ...
    // com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "lane_id[%d]valid[%d]wid[%0.3g]start[%0.3g]end[%0.3g](%.3g,%.3g,%.3g,%.3g)", 
    //                           lane_mark.id, lane_mark.view_range_valid, lane_mark.mark_width, lane_mark.view_range_start,lane_mark.view_range_end, 
    //                           lane_mark.c0, lane_mark.c1, lane_mark.c2, lane_mark.c3);
    // std::cout << "LaneMark >>>> " << string_buffer_ << std::endl;
...
#if ENABLE_DISPLAY_CAMERA_TEXT
    glColor3d(0.0, 0.6, 0.6);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "hd[%0.3g][%d](%.3g,%.3g,%.3g,%.3g)age[%d]pe[%0.3g]t[%d]", 
                              obstacle.heading, obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y,obstacle.age,obstacle.scale_change,obstacle.type);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    //std::cout << "preprocess>>>>" << string_buffer_ << std::endl;
#endif
...
#if ENABLE_DISPLAY_MAIN_RADAR_TEXT
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d](%.3g,%.3g,%.3g,%.3g)", 
                            obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    glColor3d(0.6, 0.6, 0.0);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    // std::cout << "main radar>>>>" << string_buffer_ << std::endl;
#endif


    //    com_snprintf(string_buffer_, sizeof(string_buffer_)-1,
    //                 "id(%d) vx(%0.1f) vy(%0.1f)",
    //                 obstacle.id,
    //                 (obstacle.rel_v_x+module_info_.chassis_info.v)*3.6,
    //                 obstacle.rel_v_y*3.6);
    //    renderText(obstacle.x-view_param_.origin_coor.x,
    //               obstacle.y-view_param_.origin_coor.y,
    //               0.0 , string_buffer_, QFont("System", 10));
  }
...
#if ENABLE_DISPLAY_CAMERA_TEXT
    glColor3d(0.0, 0.6, 0.6);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "hd[%0.3g][%d](%.3g,%.3g,%.3g,%.3g,%.3g,%.3g)age[%d]pe[%0.3g]t[%d]", 
                              obstacle.heading, obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y,obstacle.accel_x,obstacle.accel_y,obstacle.age,obstacle.scale_change,obstacle.type);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    //std::cout << "preprocess>>>>" << string_buffer_ << std::endl;
#endif
...
*显示预处理后的数据
*/
void WidgetMap::DrawObstacleMaxieyeCameraAfterPreprocessList() {
  const ad_msg::ObstacleCameraList& obstacle_list =
      module_info_.obstacle_maxieye_camera_after_preprocess_list;
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    DrawCross10(obstacle.x,obstacle.y,1.4);
...
//JM预处理数据
void WidgetMap::DrawObstacleJMFrontList()
{
  DrawObstacleJMCameraList(module_info_.obstacle_jm_front_list, QColor(	0, 255, 255));
}

void WidgetMap::DrawObstacleJMFrontLeftList()
{
  DrawObstacleJMCameraList(module_info_.obstacle_jm_front_left_list, QColor(0, 255, 0));
}

void WidgetMap::DrawObstacleJMFrontRightList()
{
  DrawObstacleJMCameraList(module_info_.obstacle_jm_front_right_list, QColor(0, 255, 0));
}

void WidgetMap::DrawObstacleJMRearLeftList()
{
  DrawObstacleJMCameraList(module_info_.obstacle_jm_rear_left_list, QColor(0, 255, 0));
}

void WidgetMap::DrawObstacleJMRearRightList()
{
  DrawObstacleJMCameraList(module_info_.obstacle_jm_rear_right_list, QColor(255, 97, 0));
}

void WidgetMap::DrawObstacleJMCameraList(const ad_msg::ObstacleCameraList& obstacle_list , const QColor& color){
...
绘制预处理后的侧向毫米波雷达
*/
void WidgetMap::DrawObstacleRadarLeftFwList()
{
  DrawObstacleAroundRadarList(module_info_.obstacle_anngic_radar_front_left_list, QColor(	255,69,0));
}

void WidgetMap::DrawObstacleRadarRightFwList()
{
  DrawObstacleAroundRadarList(module_info_.obstacle_anngic_radar_front_right_list, QColor(	210,105,30));
}

void WidgetMap::DrawObstacleRadarLeftBwList()
{
  DrawObstacleAroundRadarList(module_info_.obstacle_anngic_radar_rear_left_list, QColor(	255,127,80));
}

void WidgetMap::DrawObstacleRadarRightBwList()
{
  DrawObstacleAroundRadarList(module_info_.obstacle_anngic_radar_rear_right_list, QColor(	205,0,92));
}


void WidgetMap::DrawObstacleAroundRadarList(const ad_msg::ObstacleRadarList& obstacle_list , const QColor& color)
{
  qglColor(color);
  glLineWidth(4.0f);
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    DrawArrow(obstacle.x, obstacle.y, obstacle.yaw_rate, 1.5,  1); 
...
#if ENABLE_DISPLAY_HANDLED_AROUND_RADAR_TEXT
    glColor3d((double)(color.red())/255.0, (double)(color.green())/255.0, (double)(color.blue())/255.0);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    //std::cout << "preprocess>>>>" << string_buffer_ << std::endl;
#endif
...
void WidgetMap::DrawObstacleAnngicRadarFrontLeftList() {
  const ad_msg::ObstacleRadarList& obstacle_list =
      module_info_.obstacle_radar_left_fw_filter_list;

  qglColor(QColor(0, 64, 0));
  glLineWidth(2.0f);
...
#if ENABLE_DISPLAY_AROUND_RADAR_TEXT
    glColor3d(0, 64.0/255.0, 0.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d]ch[%.3g](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,module_info_.chassis_info.v,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    // LOG_INFO(2) << "Left FW Around Radar" << string_buffer_;
#endif

    //printf("Anngic[%d](%.3g,%.3g,%.3g,%.3g)\n", obstacle.id, obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
  }  
}

void WidgetMap::DrawObstacleAnngicRadarFrontRightList() {
  const ad_msg::ObstacleRadarList& obstacle_list =
      module_info_.obstacle_radar_right_fw_filter_list;
...
#if ENABLE_DISPLAY_AROUND_RADAR_TEXT
    glColor3d(0, 128.0/255.0, 0.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d]ch[%.3g](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,module_info_.chassis_info.v,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    // LOG_INFO(2) << "Right FW Around Radar" << string_buffer_;
#endif
...
void WidgetMap::DrawObstacleAnngicRadarRearLeftList() {
  const ad_msg::ObstacleRadarList& obstacle_list =
      module_info_.obstacle_radar_left_bw_filter_list;
...
#if ENABLE_DISPLAY_AROUND_RADAR_TEXT
    glColor3d(0, 192.0/255.0, 0.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d]ch[%.3g](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,module_info_.chassis_info.v,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    // LOG_INFO(2) << "Left BW Around Radar" << string_buffer_;
...
void WidgetMap::DrawObstacleAnngicRadarRearRightList() {
  const ad_msg::ObstacleRadarList& obstacle_list =
      module_info_.obstacle_radar_right_bw_filter_list;
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d]ch[%.3g](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,module_info_.chassis_info.v,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    // LOG_INFO(2) << "Right BW Around Radar" << string_buffer_;
#endif
...
#if ENABLE_DISPLAY_VISUAL_CONTROL_TEXT

    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_F[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
void WidgetMap::DrawObstacleVisualControlFrontLeftList() {
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_FL[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
void WidgetMap::DrawObstacleVisualControlFrontRightList() {
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_FR[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
void WidgetMap::DrawObstacleVisualControlRearLeftList() {
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_RL[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
void WidgetMap::DrawObstacleVisualControlRearRightList() {
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_RR[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
void WidgetMap::DrawObstacleVisualControlRearList() {
  ...
  for (Int32_t i = 0; i < obstacle_num; ++i) {
    ...
    glColor3d(0, 1.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "V_R[%d](%.3g,%.3g,%.3g,%.3g)", 
                              obstacle.id,obstacle.x, obstacle.y, obstacle.v_x, obstacle.v_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
...
#if ENABLE_DISPLAY_FUSION_TEXT
    //glColor3d(1.0, 0.0, 1.0);

    //if(fabs(obstacle.y)<5.0){
    qglColor(QColor(61, 89, 171));
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[%d]cf[%d]type[%d]d[%d]pt[%d]hd[%.3f](%.3f, %.3f,%.3f,%.3f,%.3f,%.3f)", 
                              obstacle.id, obstacle.confidence, obstacle.type, obstacle.dynamic, obstacle.perception_type,obstacle.obb.heading, 
                              obstacle.x, obstacle.y, obstacle.v_x*3.6, obstacle.v_y*3.6, obstacle.a_x, obstacle.a_y);
    renderText(obstacle.x,obstacle.y, 0, string_buffer_);
    //printf("(<<fusion>>id:%d x:%f, y:%f, v_x:%f, v_y:%f, conf: %d, type:%d, dynamic:%d, perception_type:%d, heading:%.3f)\n",obstacle.id, obstacle.x, obstacle.y, obstacle.v_x*3.6, obstacle.v_y*3.6, (Int32_t)obstacle.confidence, obstacle.type, obstacle.dynamic, obstacle.perception_type, obstacle.obb.heading);
    //}
...
#if ENABLE_DISPLAY_CHASSIS_INFO

    glColor3d(0, 100/255.0, 1.0);
    com_snprintf(string_buffer_, MAX_STRING_BUFF_SIZE-1, "[chassis](v:%.3g)", 
                              module_info_.chassis_info.v);
    renderText(10, 80, 0, string_buffer_);
    // LOG_INFO(2) << "CHASSIS INFO" << string_buffer_;
