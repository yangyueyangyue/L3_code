...
class TaskRecvSensorsInfosAndProcessor;
...
class TaskManager : public Task {
 public:
  ...
  enum {
    ...
    START_ERROR_FAILED_TO_START_SENSORS_FUSION,
    START_ERR
  };
  Int32_t Start();
  bool Stop();
...
private:
  ...
  std::unique_ptr<perception::sensorsfusion::TaskRecvSensorsInfosAndProcessor> task_sensor_fusion_;
