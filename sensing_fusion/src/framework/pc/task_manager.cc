...
#include "sensors_fusion/task_recv_sensors_infos_and_process.h"
...
#include "common/mpu_inc/map_geofence_handler.h"
...
TaskManager::TaskManager(int argc, char** argv, const std::string &work_space)
...
#if (ENABLE_ROS_NODE)
  ros_node_.reset(new RosNode(argc, argv, "sensing_fusion"));
#endif
#if (ENABLE_LCM_NODE)
  ...
  Int32_t communication_mode = 0;            //控制发送模式
  ...
  task_sensor_fusion_.reset(new perception::sensorsfusion::TaskRecvSensorsInfosAndProcessor(this));

//  printf("$$$$$$$$$$$01\n");
//  task_proc_sensors_fusion_data_.reset(new perception::sensorsfusion::goyu::SensorsFusionMainstreamImpl()); // Sensors fusion
...
Int32_t TaskManager::Start() {
  ...
  ret = task_sensor_fusion_->Start();
  if (false == ret) {
      Stop();
      LOG_ERR << "[Error]Failed to start sensor fusion.";
      return false;
  }
...
bool TaskManager::HandleMessage(const Message& msg, Task* sender) {
  ...
  case MSG_ID_RECV_SENSORS_FUSION_DATA: {
    ...
    msg_sender_->SendObstacleFusionList(*message.fusion_obstacle_list());
  ...
  case MSG_ID_RECV_ANNGIC_RADAR_OBJECTS_FRONT_LEFT: {
    ...
    shared_data->SetObstacleRadarLeftFwFilterList(*message.obstacle_list());
    // Monitor
  ...
  case MSG_ID_RECV_ANNGIC_RADAR_OBJECTS_FRONT_RIGHT: {
    const MessageRecvRadarData& message = 
        dynamic_cast<const MessageRecvRadarData&>(msg);
    shared_data->SetObstacleRadarRightFwFilterList(*message.obstacle_list());
    // Monitor
  ...
  case MSG_ID_RECV_ANNGIC_RADAR_OBJECTS_REAR_LEFT: {
    const MessageRecvRadarData& message = 
        dynamic_cast<const MessageRecvRadarData&>(msg);
    shared_data->SetObstacleRadarLeftBwFilterList(*message.obstacle_list());
    // Monitor
  ...
  case MSG_ID_RECV_ANNGIC_RADAR_OBJECTS_REAR_RIGHT: {
    const MessageRecvRadarData& message = 
        dynamic_cast<const MessageRecvRadarData&>(msg);
    shared_data->SetObstacleRadarRightBwFilterList(*message.obstacle_list());
    // Monitor
  ...
  case (MSG_ID_PERCEPTION_DEBUG):{
        const MessagePerceptionDebug& message =
        dynamic_cast<const MessagePerceptionDebug&>(msg);
    msg_sender_->SendPerceptionDebug(*(message.perception_debug()));
  ...
  case (MSG_ID_RECV_MPU_STATE_DATA): {
    ...
    exp_map_t::MapGeofenceHandler::ConvertMpuStateToMapGeofenceInfo(*message.mpu_state(),geofence_info_);
    shared_data->SetMapGeofenceInfo(geofence_info_);
    // Monitor
