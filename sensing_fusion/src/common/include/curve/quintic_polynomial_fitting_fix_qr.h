 ...
 ******************************************************************************
 * @file       quintic_polynomial_fitting_fix_qr.h
 * @brief      5次多项式拟合
 * @details    根据输入的带参数（沿着曲线的长度）的二维点，使用最小二乘法拟合5次
 *             多项式曲线。
 *             单次拟合时，两个相邻拟合点的间距需要是固定的。
 *             这样中间计算过程中需要用到的Q矩阵和R矩阵就可以
 *             仅计算一次。
 *
 * @author     pengc,boc
 * @date       2020.05.15
 ...
 * <tr><td>2020/05/08  <td>1.0      <td>pengc,boc <td>First edition
 * </table>
 *
 ******************************************************************************/

#ifndef PHOENIX_COMMON_QUINTIC_POLYNOMIAL_FITTING_FIX_QR_H_
#define PHOENIX_COMMON_QUINTIC_POLYNOMIAL_FITTING_FIX_QR_H_
...
#define ENABLE_QUINTIC_POLYNOMIAL_FITTING_FIX_QR_TRACE (0)
...
/**
 * @class QuinticPolynomialFittingFixQR
...
public:
  ...
  QuinticPolynomialFittingFixQR() {
    ...
    length_ = 0;
    scale_s_ = 0;
    qr_matrix_calculated_no_constraint_ = false;
    qr_matrix_calculated_has_constraint_ = false;
    InitMatrixQR();
  }
...
//  Matrix<Scalar, MaxPointNum, MaxPointNum> mat_orth_;
  // 根据输入的待拟合的点集构造的系数矩阵
  Matrix<Scalar, MaxPointNum, PARAM_DIM> mat_coef_no_constraint_;
  // 保存对系数矩阵进行正交分解后的正交矩阵
  Matrix<Scalar, MaxPointNum, MaxPointNum> mat_orth_no_constraint_;
  // 根据输入的待拟合的点集构造的系数矩阵
  Matrix<Scalar, MaxPointNum, PARAM_DIM> mat_coef_has_constraint_;
  // 保存对系数矩阵进行正交分解后的正交矩阵
  Matrix<Scalar, MaxPointNum, MaxPointNum> mat_orth_has_constraint_;
  // 不带起点约束的QR矩阵是否已经计算了
  bool qr_matrix_calculated_no_constraint_;
  // 带起点约束的QR矩阵是否已经计算了
  bool qr_matrix_calculated_has_constraint_;
  // 参数化曲线系数向量x
...
template<typename Scalar, Int32_t MaxPointNum>
bool QuinticPolynomialFittingFixQR<Scalar, MaxPointNum>::
...
#if ENABLE_QUINTIC_POLYNOMIAL_FITTING_FIX_QR_TRACE
  std::cout << "########### QuinticPolynomialFittingFixQR::"
               "ConstructWithoutConstraint ############>"
            << std::endl;
  ...
  vec_x.SetBlockSize(point_num, 1);
  vec_y.SetBlockSize(point_num, 1);
  coef_a_.SetBlockSize(point_num, 1);
  coef_b_.SetBlockSize(point_num, 1);
  ...
  for (Int32_t i = 0; i < point_num; ++i) {
    std::cout << "[" << i << "] ";
    for (Int32_t j = 0; j < PARAM_DIM; ++j) {
      std::cout << mat_coef_no_constraint_(i, j) << "\t";
    }
    std::cout << std::endl;
  }
#endif
  ...
  for (Int32_t i = 0; i < point_num; ++i) {
    std::cout << "[" << i << "] ";
    for (Int32_t j = 0; j < point_num; ++j) {
      std::cout << mat_orth_no_constraint(i, j) << "\t";
    }
    std::cout << std::endl;
  }
  ...
  Mat_Transpose(mat_orth_no_constraint, mat_orth_tran);
  Mat_Mul(mat_orth_no_constraint, mat_orth_tran, mat_identity);
  ...
  Mat_Mul(mat_orth_tran, mat_coef_no_constraint_, mat_recon);
  ...
  if (false == Mat_CalcLinearEquationFromQR(
        mat_coef_no_constraint_, mat_orth_no_constraint_, vec_x, coef_a_)) {
    LOG_ERR << "Failed to evaluate the coefficient a of the polynomial curve.";
    return false;
  }
  if (false == Mat_CalcLinearEquationFromQR(
        mat_coef_no_constraint_, mat_orth_no_constraint_, vec_y, coef_b_)) {
...
//  ddy = (ndy - dy) / ds;
  ...
  vec_x.SetBlockSize(point_num-1, 1);
  vec_y.SetBlockSize(point_num-1, 1);
  coef_a_.SetBlockSize(point_num-1, 1);
  coef_b_.SetBlockSize(point_num-1, 1);
  ...
  for (Int32_t i = 0; i < (point_num-1); ++i) {
    std::cout << "[" << i << "] ";
    for (Int32_t j = 0; j < param_dim; ++j) {
      std::cout << mat_coef_has_constraint_(i, j) << "\t";
    }
    std::cout << std::endl;
  }
#endif
  ...
  for (Int32_t i = 0; i < (point_num-1); ++i) {
    std::cout << "[" << i << "] ";
    for (Int32_t j = 0; j < (point_num-1); ++j) {
      std::cout << mat_orth_has_constraint_(i, j) << "\t";
    }
    std::cout << std::endl;
  }
  ...
  Mat_Transpose(mat_orth_has_constraint_, mat_orth_tran);
  Mat_Mul(mat_orth_has_constraint_, mat_orth_tran, mat_identity);
  std::cout << "The product of multiplying Q by it's transpose is :"
            << std::endl;
  for (Int32_t j = 0; j < (point_num-1); ++j) {
    std::cout << "[" << j << "]\t";
  ...
  Mat_Mul(mat_orth_tran, mat_coef_has_constraint_, mat_recon);
  std::cout << "The reconstructed matrix is :" << std::endl;
  for (Int32_t j = 0; j < param_dim; ++j) {
    std::cout << "[" << j << "]\t";
  }
  ...
  // 使用将系数矩阵分解后的QR矩阵求解多项式曲线的系数矩阵
  if (false == Mat_CalcLinearEquationFromQR(
        mat_coef_has_constraint_, mat_orth_has_constraint_, vec_x, coef_a_)) {
    LOG_ERR << "Failed to evaluate the coefficient a of the polynomial curve.";
    return false;
  }
  if (false == Mat_CalcLinearEquationFromQR(
        mat_coef_has_constraint_, mat_orth_has_constraint_, vec_y, coef_b_)) {
    LOG_ERR << "Failed to evaluate the coefficient b of the polynomial curve.";
    return false;
  }
...
template<typename Scalar, Int32_t MaxPointNum>
bool QuinticPolynomialFittingFixQR<Scalar, MaxPointNum>::CalcSamplePoint(
...
template<typename Scalar, Int32_t MaxPointNum>
void QuinticPolynomialFittingFixQR<Scalar, MaxPointNum>::CalcSamplePointInteral(
    const Scalar s, Vec2d* point) {
...
template<typename Scalar, Int32_t MaxPointNum>
Scalar QuinticPolynomialFittingFixQR<Scalar, MaxPointNum>::CalcDerivative(
...
template<typename Scalar, Int32_t MaxPointNum>
bool QuinticPolynomialFittingFixQR<Scalar, MaxPointNum>::InitMatrixQR() {
  const Int32_t point_num = MaxPointNum;
  // 计算无起点约束的QR矩阵
  for (Int32_t i = 0; i < point_num; ++i) {
    if (!qr_matrix_calculated_no_constraint_) {
      Scalar s = i / (0.2f * (point_num - 1));
      for (Int32_t j = 0; j < PARAM_DIM; ++j) {
        if (0 == j) {
          mat_coef_no_constraint_(i, j) = 1;
        } else if (1 == j) {
          mat_coef_no_constraint_(i, j) = s;
        } else {
          mat_coef_no_constraint_(i, j) = s * mat_coef_no_constraint_(i, j - 1);
        }
      }
    }
  }
  ...
  if (!qr_matrix_calculated_no_constraint_) {
    // 设置矩阵的行数为实际的点的数量
    mat_coef_no_constraint_.SetBlock(0, 0, point_num, PARAM_DIM);
    mat_orth_no_constraint_.SetBlock(0, 0, point_num, point_num);
  }

  // 将系数矩阵进行QR分解
  if (!qr_matrix_calculated_no_constraint_) {
    if (false == Mat_HouseholderQR(mat_coef_no_constraint_,
                                   mat_orth_no_constraint_)) {
      LOG_ERR << "The coefficient matrix is invalid.";
      return (false);
    }
    qr_matrix_calculated_no_constraint_ = true;
  }
...
#if (QUINTIC_FITTING_CONSTRAINT_NUM == 1)
  for (Int32_t i = 0; i < (point_num-1); ++i) {
    if (!qr_matrix_calculated_has_constraint) {
      // Scalar s = (path_points[i+1].s - start_s_) / scale_s_;
      Scalar s = (i + 1) / (0.2f * (point_num - 1));
      for (Int32_t j = 0; j < param_dim; ++j) {
        if (0 == j) {
          mat_coef_has_constraint_(i, j) = s;
        } else {
          mat_coef_has_constraint_(i, j) = s * mat_coef_has_constraint_(i, j-1);
        }
      }
    }
    vec_x(i) = path_points[i + 1].point.x() / scale_s_ - x;
  ...
  for (Int32_t i = 0; i < (point_num - 1); ++i) {
    Scalar s = (i + 1) / (0.2F * (point_num - 1));
    if (!qr_matrix_calculated_has_constraint_) {
      for (Int32_t j = 0; j < param_dim; ++j) {
        if (0 == j) {
          mat_coef_has_constraint_(i, j) = s * s;
        } else {
          mat_coef_has_constraint_(i, j) =
              s * mat_coef_has_constraint_(i, j - 1);
        }
      }
    }
  }
  ...
  for (Int32_t i = 0; i < (point_num-1); ++i) {
    if (!qr_matrix_calculated_has_constraint) {
      ...
      for (Int32_t j = 0; j < param_dim; ++j) {
        if (0 == j) {
          mat_coef_has_constraint_(i, j) = s * s * s;
        } else {
          mat_coef_has_constraint_(i, j) =
              s * mat_coef_has_constraint_(i, j - 1);
        }
  ...
  // 设置矩阵的行数为实际的点的数量
  if (!qr_matrix_calculated_has_constraint_) {
    mat_coef_has_constraint_.SetBlock(0, 0, point_num - 1, param_dim);
    mat_orth_has_constraint_.SetBlock(0, 0, point_num - 1, point_num - 1);
  }

  // 将系数矩阵进行QR分解
  if (!qr_matrix_calculated_has_constraint_) {
    if (false == Mat_HouseholderQR(mat_coef_has_constraint_,
                                   mat_orth_has_constraint_)) {
      LOG_ERR << "The coefficient matrix is invalid.";
      return (false);
    }
    qr_matrix_calculated_has_constraint_ = true;
  }
...
#endif  // PHOENIX_COMMON_QUINTIC_POLYNOMIAL_FITTING_FIX_QR_H_
